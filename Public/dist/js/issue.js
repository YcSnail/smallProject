...
dd.ready(function() {
	...
	var issue = Common.getCookie('issueorder');
	if (issue != '' && issue != null) {
		$('#lastorderid').text('订单号' + issue + '已经出库完成');
	}
	...
	$('#btnIssue').on('click', function() {
		...
		json('get', url, params, function(data) {
			removeLoading();
			if (data.code == 0) {
				mui.toast('出库成功！',{ duration:'long', type:'div' });
				alert('出库成功！');
				Common.setCookie('issueorder', orderid, {expires: 30, path: '/'});
				location.href = '/jinco/index.php/welcome/issue';
			} else {
				alert('出库失败！' + "\n\n" + data.message);
			}
		});
	...
	$('#btnAllocate').on('click', function() {
		...
		$('.package').each(function(index) {
			...
			if (pallet != '' && pallet != '{0}') {
				...
				params['pallet'][n] = pallet + '|' + from + '|' + to;
				n++;
			}
		});
